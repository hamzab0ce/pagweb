  ...
  return (
    <li onClick={onClick} className="max-w-2xs cursor-pointer relative overflow-hidden">
      <div className="flex flex-col items-center justify-center rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 ease-in-out">
        <img src={game.cover_url} alt={game.title} className="object-contain h-72" />
        ...
        <span className="absolute bg-zinc-900/40 border border-zinc-600 backdrop-blur rounded-md m-2 font-sans top-0 right-0 p-1 text-white">{game.platform}</span>
      </div>
    </li>
  );
}
